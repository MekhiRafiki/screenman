...
export default function Stage() {
    ...
    const currentText = stageProposals.length > 0 ? stageProposals[0].text : 'Following along...';
    const textSize = getTextSize(currentText);

    return (
        <div className="flex flex-col gap-6">
            <div className="bg-white/10 backdrop-blur-sm p-8 rounded-lg shadow-lg">
                <p className={`${textSize} font-bold text-white text-center leading-relaxed mb-8 transition-all duration-300`}>
                    {currentText}
                </p>
